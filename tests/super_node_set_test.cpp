...
TEST(SuperNodeSetTest, NeighborEdgeCounts_Basic) {
    // verifies that nneigbor_edge_counts[u_super] citains the expected neighbor per edge count map
    const Graph g = {{1, 2}, {0}, {0}};
    SuperNodeSet super_nodes(g);

    auto neigbour_edge_counts_0 = super_nodes.get_neighbor_edge_counts(0);

    
    EXPECT_EQ(neigbour_edge_counts_0.size(), 2);
    EXPECT_EQ(neigbour_edge_counts_0.at(1), 1);
    EXPECT_EQ(neigbour_edge_counts_0.at(2), 1);
}
...
TEST(SuperNodeSetTest, NeighborEdgeCounts_UV_WithInternalEdge) {
    ...
    // The edge(s) between u and v get stored as "internal edges" under u_super (key 0).
    // Here there is one 0-1 edge, and counts(1) also contributes one 1->0 edge,
    // so w[0] ends up being 2 as derived above.
    EXPECT_TRUE(w_counts.contains(0));
    EXPECT_EQ(w_counts.at(0), 2);

    // // The shared neighbor 2 should have count 2 (one from 0->2 and one from 1->2)
...
TEST(SuperNodeSetTest, AccumulateCost_WithInternalEdges) {
    ...
    neighbor_edge_counts.emplace(0, 2);  // internal edge: raw=2 => unique=1, cart=1 => min(1-1+1=1, 1)=1
    neighbor_edge_counts.emplace(1, 1);  // normal edge: raw=1 => unique=1, cart=1 => min(1,1)=1

    const double cost = super_nodes.accumulate_cost(/*u*/0, /*num_vertices_u*/2, neighbor_edge_counts);
...
TEST(SuperNodeSetTest, AccumulateCost_NoInternalEdges) {
    ...
    const double cost = super_nodes.accumulate_cost(/*u*/0, /*num_vertices_u*/1, neighbor_edge_counts);
    EXPECT_DOUBLE_EQ(cost, 2.0);
}
...
TEST(SuperNodeSetGetCostTest, MergeCost_NotAffectingOtherNodes) {
    ...
    EXPECT_DOUBLE_EQ(super_nodes.get_cost(2), 2.0); // a hypotetical merge does not affect nbr nodes
}
...
TEST(SuperNodeSetTest, Merge_Basic) {
    ...
    // Num vertices
    EXPECT_EQ(s.get_num_vertices(0), 2);
    EXPECT_EQ(s.get_num_vertices(1), 2);
...
TEST(SuperNodeSetTest, Merge_AllIntoOne) {
    ...
    EXPECT_EQ(s.get_num_vertices(0), 4);
...
TEST(SuperNodeSetTest, Merge_SelfMerge) {
    ...
    EXPECT_EQ(s.get_num_vertices(0), 1);
    EXPECT_EQ(s.get_num_vertices(1), 1);
