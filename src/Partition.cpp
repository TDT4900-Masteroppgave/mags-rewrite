#include "mags/Partition.h"
namespace mags {

Partition::Partition(const size_t n) : dsu(n) {}

void Partition::finalize(const Graph &graph) {
  const int n = static_cast<int>(graph.size());
  super_nodes.clear();
  members.assign(n, {});
  edge_counts.assign(n, {});

  // Identify roots and reserve memory
  for (NodeID u = 0; u < n; ++u) {
    NodeID root = dsu.find(u);
    if (root == u) {
      super_nodes.push_back(root);
      members.at(root).reserve(dsu.size(root));
    }
    members.at(root).push_back(u);
  }

  // Compute original edge counts
  for (NodeID u = 0; u < n; ++u) {
    for (const NodeID v : graph.at(u)) {
      // Only process an edge (u, v) once
      if (u <= v) {
        NodeID root_u = dsu.find(u);
        NodeID root_v = dsu.find(v);
        if (root_u > root_v)
          std::swap(root_u, root_v);
        edge_counts.at(root_u)[root_v]++;
      }
    }
  }
}

const SuperNodes &Partition::get_super_nodes() const { return super_nodes; }

const SuperNodeMembers &Partition::get_members() const { return members; }

const EdgeCounts &Partition::get_edge_counts() const {
  return edge_counts;
}

long long Partition::get_cartesian_product(const NodeID super_u,
                                           const NodeID super_v) const {
  const auto size_u = static_cast<long long>(members.at(super_u).size());
  if (super_u == super_v) {
    return size_u * (size_u - 1) / 2;
  }

  const auto size_v = static_cast<long long>(members.at(super_v).size());
  return size_u * size_v;
}

NodeID Partition::find_super_node(const NodeID u) const { return dsu.find(u); }

void Partition::merge(const NodeID u, const NodeID v) {
  dsu.unite(u, v);
}

} // namespace mags