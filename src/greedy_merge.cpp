...
void replace(const NodeID v, const SuperNodeSet &super_nodes_set,
  ...
  for (auto [candidate_node, saving_score] : candidate_set[v]) {
    ...
    if (!candidate_set[u_super].contains(candidate_node)) {
      ...
      // priority queue
      candidate_set[u_super][candidate_node] =
          candidate_set[candidate_node][u_super] = -1;
...
void evaluate(NodeID u, NodeID v, SuperNodeSet &super_nodes_set,
              ...
              const double threshold_new_saving_score = -0.03) {
  // compute new saving
  const double new_saving_score = super_nodes_set.saving(u, v);

  // early termination
  if (candidate_set[u][v] == new_saving_score)
    return;
...
void remove_candidate_v(NodeID u, NodeID v, CandidateSet &candidate_set,
                        PriorityQueue &priority_queue) {
...
SuperNodeSet greedy_merge(Graph const &graph, int num_iterations,
  ...
  for (int i = 1; i <= num_iterations; i++) {
    ...
    phmap::flat_hash_set<NodeID>
        batch_to_update; // set containing unique nodes and their neighbors
    ...
    for (const auto &[s, node_pair] : priority_queue) {
      ...
      if (s < detail::merge_threshold(i, num_iterations, start_threshold,
      ...
      if (super_nodes_set.saving(u, v) >=
          detail::merge_threshold(i, num_iterations, start_threshold,
                                  end_threshold, ratio_base)) {
