...
SuperNodeSet merge(
    ...
    for (int i = 0; i < num_iterations; i++) {
        ...
        for (const auto& [s, node_pair] : priority_queue) {
            ...
            if (s < merge_threshold(i, num_iterations)) continue;
            ...
            if (super_nodes_set.saving(u, v) >= merge_threshold(i, num_iterations)) {
        ...
        for (const mags::NodeID& u : batch_to_update) {
            ...
            for (auto v : to_remove_suv) {
                remove_candidate_v(u, v, candidate_set, priority_queue);
            }
            for (auto& [v, saving_score] : to_update_suv) {
                update_candidate_v(u, v, saving_score, candidate_set, priority_queue);
            }
